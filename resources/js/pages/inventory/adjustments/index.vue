        ...
        <div class="card custom-card w-100">
          <div class="card-header setings-header">
            <div class="col-xl-4 col-4">
              <h3 class="card-title">
                {{ $t("inventory.adjustments.index.page_title") }}
              </h3>
            </div>
            <div class="col-xl-8 col-8 float-right text-right">
              <div class="btn-group c-w-100">
                ...
                <a href="/inventory-adjustments/pdf" v-tooltip="$t('common.export_pdf')" class="btn btn-secondary">
                  <i class="fas fa-download"></i>
                </a>
                <a @click="print" v-tooltip="$t('common.print_table')" class="btn btn-info">
                  <i class="fas fa-print"></i>
                </a>
                <router-link v-if="$can('adjustment-create')" :to="{ name: 'adjustments.create' }"
            ...
            <div class="table-responsive table-custom mt-3" id="printMe">
              <table class="table">
                <thead>
                  <tr>
                    <th>{{ $t("common.s_no") }}</th>
                    <th>{{ $t("inventory.common.adjustment_no") }}</th>
                    <th>{{ $t("common.reason") }}</th>
                    <th>{{ $t("common.date") }}</th>
                    <th>{{ $t("common.status") }}</th>
                    <th v-if="$can('adjustment-edit') ||
                ...
                <tbody>
                  <tr v-show="items.length" v-for="(data, i) in items" :key="i">
                    ...
                    <td v-if="$can('adjustment-edit') ||
                        $can('adjustment-delete') ||
                        $can('adjustment-view')
                        " class="text-right no-print">
                      <div class="btn-group">
                        <router-link v-if="$can('adjustment-view')" v-tooltip="$t('common.view')" :to="{
                          name: 'adjustments.show',
                          params: { slug: data.slug },
                        }" class="btn btn-primary btn-sm">
                          <i class="fas fa-eye" />
                        </router-link>
                        <router-link v-if="$can('adjustment-edit')" v-tooltip="$t('common.edit')" :to="{
                          name: 'adjustments.edit',
                          params: { slug: data.slug },
                        }" class="btn btn-info btn-sm">
                          <i class="fas fa-edit" />
                        </router-link>
                        <a v-if="$can('adjustment-delete')" v-tooltip="$t('common.delete')" href="#"
...
export default {
  ...
  metaInfo() {
    return { title: this.$t("inventory.adjustments.index.page_title") };
  },
  components: {
    DateRangePicker,
  },
  data: () => ({
    breadcrumbsCurrent: "inventory.adjustments.index.breadcrumbs_current",
    breadcrumbs: [
      {
        name: "inventory.adjustments.index.breadcrumbs_first",
        url: "home",
      },
      {
        name: "inventory.adjustments.index.breadcrumbs_second",
        url: "products.index",
      },
      {
        name: "inventory.adjustments.index.breadcrumbs_active",
        url: "",
      },
    ],
    query: "",
  ...
  computed: {
    ...
    exportUrl() {
      ...
      return `/inventory-adjustments/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}`;
    },
  },
  watch: {
    // watch search data
  ...
  methods: {
    ...
    async deleteData(slug) {
      Swal.fire({
        ...
        text: this.$t("inventory.adjustments.index.delete_warning"),
        type: "warning",
