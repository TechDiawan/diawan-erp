        ...
        <div class="card custom-card w-100">
          <div class="card-header setings-header">
            <div class="col-xl-4 col-4">
              <h3 class="card-title">
                {{ $t("loans.payments.index.page_title") }}
              </h3>
            </div>
            <div class="col-xl-8 col-8 float-right text-right">
              <div class="btn-group c-w-100">
                ...
                <a href="/loan-payments/pdf" v-tooltip="$t('common.export_pdf')" class="btn btn-secondary">
                ...
                <tbody>
                  <tr v-show="items.length" v-for="(data, i) in items" :key="i">
                    ...
                    <td v-if="$can('loan-payment-edit') ||
                        ...
                        " class="text-right no-print">
                      <div class="btn-group">
                        <router-link v-if="$can('loan-payment-view')" v-tooltip="$t('common.view')" :to="{
                          name: 'loanPayments.show',
                          params: { slug: data.slug },
                        }" class="btn btn-primary btn-sm">
                          <i class="fas fa-eye" />
                        </router-link>
                        <router-link v-if="$can('loan-payment-edit')" v-tooltip="$t('common.edit')" :to="{
                          name: 'loanPayments.edit',
                          params: { slug: data.slug },
                        }" class="btn btn-info btn-sm">
                          <i class="fas fa-edit" />
                        </router-link>
                        <a v-if="$can('loan-payment-delete')" v-tooltip="$t('common.delete')" href="#"
...
export default {
  ...
  metaInfo() {
    return { title: this.$t("loans.payments.index.page_title") };
  },
  components: {
    DateRangePicker,
  },
  data: () => ({
    breadcrumbsCurrent: "loans.payments.index.breadcrumbs_current",
    breadcrumbs: [
      {
        name: "loans.payments.index.breadcrumbs_first",
        url: "home",
      },
      {
        name: "loans.payments.index.breadcrumbs_second",
        url: "",
      },
      {
        name: "loans.payments.index.breadcrumbs_active",
        url: "",
      },
    ],
    query: "",
  ...
  methods: {
    ...
    async deleteData(slug) {
      Swal.fire({
        ...
        text: this.$t("loans.payments.index.delete_warning"),
        type: "warning",
            ...
            .then((response) => {
              if (response === true) {
                ...
                Swal.fire(
                  ...
                  this.$t("loans.payments.index.delete_failed"),
                  "warning"
                );
              }
            });
