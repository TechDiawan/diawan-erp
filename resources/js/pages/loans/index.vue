          ...
          <div class="card-header setings-header">
            ...
            <div class="col-xl-8 col-8 float-right text-right">
              <div class="btn-group c-w-100">
                ...
                <a href="/loans/pdf" v-tooltip="$t('common.export_pdf')" class="btn btn-secondary">
              ...
              <table class="table">
                <thead class="thead-light">
                  <tr>
                    ...
                    <th>{{ $t("loans.common.authority") }}</th>
                ...
                <tbody>
                  <tr v-show="items.length" v-for="(data, i) in items" :key="i">
                    ...
                    <td>
                      <router-link v-if="$can('loan-view')" :to="{
                        name: 'loans.show',
                    ...
                    <td>
                      <span v-if="data.authority">{{ data.authority.name }}
                      </span>
                    </td>
                    <td>
                    ...
                    <td v-if="$can('loan-edit') ||
                        $can('loan-view') ||
                        $can('loan-delete')
                        " class="text-right no-print">
                      <div class="btn-group">
                        <router-link v-if="$can('loan-view')" v-tooltip="$t('common.view')" :to="{
                          name: 'loans.show',
                          params: { slug: data.slug },
                        }" class="btn btn-primary btn-sm">
                          <i class="fas fa-eye" />
                        </router-link>
                        <router-link v-if="$can('loan-edit')" v-tooltip="$t('common.edit')" :to="{
                          name: 'loans.edit',
                          params: { slug: data.slug },
                        }" class="btn btn-info btn-sm">
                          <i class="fas fa-edit" />
                        </router-link>
                        <a v-if="$can('loan-delete')" v-tooltip="$t('common.delete')" href="#"
...
export default {
  ...
  metaInfo() {
    return { title: this.$t("loans.list.index.page_title") };
  },
  components: {
    DateRangePicker,
  },
  data: () => ({
    breadcrumbsCurrent: "loans.list.index.breadcrumbs_current",
    breadcrumbs: [
      {
        name: "loans.list.index.breadcrumbs_first",
        url: "home",
      },
      {
        name: "loans.list.index.breadcrumbs_second",
        url: "",
      },
      {
        name: "loans.list.index.breadcrumbs_active",
        url: "",
      },
    ],
    query: "",
  ...
  computed: {
    ...
    exportUrl() {
      ...
      return `/loans/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}`;
    },
  },
  watch: {
    // watch search data
  ...
  methods: {
    ...
    async deleteData(slug) {
      Swal.fire({
        ...
        text: this.$t("loans.authorities.index.delete_warning"),
        type: "warning",
            ...
            .then((response) => {
              if (response === true) {
                ...
                Swal.fire(
                  ...
                  this.$t("loans.list.index.delete_failed"),
                  "warning"
                );
              }
            });
