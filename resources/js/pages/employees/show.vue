<template>
  <div>
    ...
    <!-- breadcrumbs end -->
    <div class="row">
      <div class="col-md-12 m-auto" :class="!$can('payroll-list') && !$can('increment-list')
        ? 'col-lg-6'
        : 'col-lg-3'
        ">
        <!-- Profile Image -->
        <div class="card">
          <div class="card-body box-profile">
            ...
            <ul class="list-group list-group-unbordered mb-3">
              ...
              <li v-if="allData.user && allData.user.role" class="list-group-item">
      ...
      <!-- /.col -->
      <div v-if="$can('payroll-list') || $can('increment-list')" class="col-md-12 col-lg-9">
        <div class="card">
              ...
              <div class="col-md-8">
                <ul class="nav nav-pills">
                  <li v-if="$can('payroll-list')" class="nav-item">
                    <a class="nav-link active" href="#payroll" data-toggle="tab">{{ $t("sidebar.payroll") }}</a>
                  </li>
                  <li v-if="$can('increment-list')" class="nav-item">
                    <a @click="getEmployeeSalIncrements" class="nav-link" href="#increment" data-toggle="tab">{{
          ...
          <div class="card-body">
            <div class="tab-content">
              <div v-if="$can('payroll-list')" class="tab-pane active" id="payroll">
                <div>
                          ...
                          <tr v-show="items.length" v-for="(data, i) in items" :key="i">
                            ...
                            <td>
                              <span v-if="data.transaction &&
                                  ...
                                  ">{{ data.transaction.cashbook_account.account_number }}</span>
                            </td>
                            <td>
                              <span v-if="data.transaction">{{
                            ...
                            <td v-if="$can('payroll-edit') ||
                              ...
                              <div class="btn-group">
                                ...
                                <a v-if="$can('payroll-delete')" v-tooltip="$t('common.delete')" href="#"
                                  class="btn btn-danger btn-sm" @click="deletePayroll(data.slug)">
                  ...
                  <!-- NEW PAGINATION -->
                  <div v-if="pagination && pagination.total > 0" class="dtable-footer">
                    <div class="form-group row display-per-page">
                      <label>{{ $t("per_page") }} </label>
                      <div>
                        <select @change="updatePerPager('payroll')" v-model="perPage"
              ...
              <div v-if="$can('increment-list')" class="tab-pane" id="increment">
                <div>
                  <div class="card-body p-0 position-relative">
                    <div class="col-md-12 large-serach-box">
                      <search v-model="salIncreQuery" @reset-pagination="resetSalIncrePagination"
                    ...
                    <div class="table-responsive table-custom mt-3">
                      <table class="table">
                        ...
                        <tbody>
                          <tr v-show="allIncrements.length" v-for="(data, i) in allIncrements" :key="i">
                            <td>
                              <span v-if="salIncrePagination &&
                            ...
                            <td v-if="$can('increment-edit') ||
                                ...
                                " class="text-right">
                              <div class="btn-group">
                                <router-link v-if="$can('increment-view')" v-tooltip="$t('common.view')" :to="{
                                  name: 'increments.show',
                                  params: { slug: data.slug },
                                }" class="btn btn-primary btn-sm">
                                  <i class="fas fa-eye" />
                                </router-link>
                                <router-link v-if="$can('increment-edit')" v-tooltip="$t('common.edit')" :to="{
                                  name: 'increments.edit',
                                  params: { slug: data.slug },
                                }" class="btn btn-info btn-sm">
                                  <i class="fas fa-edit" />
                                </router-link>
                                <a v-if="$can('increment-delete')" v-tooltip="$t('common.delete')" href="#"
                                  class="btn btn-danger btn-sm" @click="deleteIncrement(data.slug)">
                                  <i class="fas fa-trash" />
                                </a>
                              </div>
                            </td>
                  ...
                  <!-- NEW PAGINATION -->
                  <div v-if="salIncrePagination && salIncrePagination.total > 0" class="dtable-footer">
                    <div class="form-group row display-per-page">
                      <label>{{ $t("per_page") }} </label>
                      <div>
                        <select @change="updatePerPager('increment-history')" v-model="perPage"
                          class="form-control form-control-sm ml-1">
                          <!-- options component -->
                          <option v-for="(option, i) in options" :key="i" :value="option.value">
                            {{ option.text }}
                    ...
                    <!-- pagination-start -->
                    <pagination v-if="salIncrePagination && salIncrePagination.last_page > 1
                      " :pagination="salIncrePagination" :offset="5" class="justify-flex-end"
                      @paginate="salIncrePaginate" />
                    <!-- pagination-end -->
                  </div>
                  <!-- NEW PAGINATION END -->
